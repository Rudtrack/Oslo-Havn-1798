Shader "VacuumShaders/Advanced Dissolve/Physically Based/Specular"  
{
	Properties {
...
#include "Assets/VacuumShaders/Advanced Dissolve/Shaders/cginc/AdvancedDissolve.cginc"
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
			...
			fixed4 s = tex2D(_SpecularMap, IN.uv_SpecularMap);

			o.Specular = _SpecularColor.rgb * s.rgb * lerp(c.a, s.a, _SpecularAlpha) * _Specular;
...
struct v2f_surf {
  ...
  float2 pack1 : TEXCOORD1; // _SpecularMap
...
#else
  DissolveVertex2Fragment(o.pos, v.texcoord.xy, v.texcoord1.xy, o.dissolveUV);
#endif
...
#else
float4 alpha = ReadDissolveAlpha(IN.pack0.xy, IN.dissolveUV, IN.worldPos);
#endif
DoDissolveClip(alpha);
