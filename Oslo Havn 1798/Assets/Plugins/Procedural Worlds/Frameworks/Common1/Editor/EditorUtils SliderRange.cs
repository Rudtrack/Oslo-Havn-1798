        ...
        public bool SliderRange(Rect position, GUIContent label, ref float start, ref float end, float minValue = 0f, float maxValue = 1f, bool invert = false) {
            ...
            if (Event.current.type == EventType.MouseDown && Event.current.button == 0) {
                ...
                if (GUIUtility.hotControl == maxControlID) {
                    ...
                    float DragDelta = Mathf.Clamp((Event.current.mousePosition.x - SliderRangeDragPos) / barRect.width, -SliderRangeDragStartEnd.x, 1f - SliderRangeDragStartEnd.y);
                    start = Mathf.Clamp(SliderRangeDragStartEnd.x + DragDelta * (maxValue - minValue), minValue, maxValue);
                    end = Mathf.Clamp(SliderRangeDragStartEnd.y + DragDelta * (maxValue - minValue), minValue, maxValue);
                    Event.current.Use();
                    changed = true;
                }
